...
@cache
def count_matches(springs: str, groups: tuple[int, ...]) -> int:
    if not groups:
    ...
    for before in range(len(springs) - after - car_int + 1):
        ...
        if all(
            ...
            count += count_matches(springs[len(possible_springs) :], cdr_ints)
    return count
...
with open("input.txt", "r") as input:
    ...
    for row in rows:
        ...
        print(int_groups_p2)
        total_p1 += count_matches(pattern, int_groups_p1)
        total_p2 += count_matches("?".join((pattern,) * 5), int_groups_p2)
